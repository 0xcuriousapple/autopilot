...
contract AutoPilotExecuteTest is Test {
    ...
    uint256 privateKeyBot = uint256(bytes32(0x1c5a1a856270f46b6007ad5a163d93ae5cf418486f4f81aa56f328bf1a5596ae));
    address public bot = 0xB5cdafA4932Ae6f5EA1d37FAD9642AB6195337DC;

    uint256 privateKeyOwner = uint256(bytes32(0x16d18a5980a191f43a457e23cfb9a72beb301b41aad581bfb33d5f127c008bd0));
    ...
    function testExecuteHappyCase() public {
        // owner
        vm.startPrank(owner);
        bytes memory calldataAutoPilot = abi.encodeWithSignature("setX(uint256)", 1);
        ...
        vm.startPrank(owner);
        autopilot.addAllowedCall(keccak256(abi.encode(address(externalContract), 10000000, calldataAutoPilot)), 10);
        vm.stopPrank();
        // bot validate signature state update
        vm.store(address(autopilot), bytes32(uint256(3)), bytes32(abi.encode(address(bot))));
        
        vm.startPrank(entryPoint);
        vm.deal(address(autopilot), 100 ether);
        autopilot.execute(address(externalContract), 10000000, calldataAutoPilot);
        assertEq(externalContract.x(), 1);   
    ...
    function testBotCheckTimeGap() public {
        ...
        vm.startPrank(owner);
        bytes memory calldataAutoPilot = abi.encodeWithSignature("setX(uint256)", 1);
        autopilot.addAllowedCall(keccak256(abi.encode(address(externalContract), 0, calldataAutoPilot)), 10);
        vm.stopPrank();

        // first call
        vm.warp(1000);
        vm.store(address(autopilot), bytes32(uint256(3)), bytes32(abi.encode(bot)));
        vm.startPrank(entryPoint);
        autopilot.execute(address(externalContract), 0, calldataAutoPilot);
        assertEq(externalContract.x(), 1);
        (bool allowed, uint96 timegap, uint128 lastCallTime) = autopilot.allowedCalls(keccak256(abi.encode(address(externalContract), 0, calldataAutoPilot)));
        assertEq(lastCallTime, 1000);
        externalContract.setX(0);

        // second call before time gap
        vm.warp(1001);
        vm.store(address(autopilot), bytes32(uint256(3)), bytes32(abi.encode(bot))); // I understnad that this not needed, doing it for clarity
        vm.expectRevert("account: timegap not reached");
        autopilot.execute(address(externalContract), 0, calldataAutoPilot);
        assertEq(externalContract.x(), 0);
        (allowed, timegap, lastCallTime) = autopilot.allowedCalls(keccak256(abi.encode(address(externalContract), 0, calldataAutoPilot)));
        assertEq(lastCallTime, 1000);
